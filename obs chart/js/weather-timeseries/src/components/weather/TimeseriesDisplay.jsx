...
const TodayHeaderCell = styled(HeaderCell)`
  width: ${props => {
    // Calculate width based on the number of time slots
    // Each time slot should take up approximately 6.25% of the total width
    // For 4 time slots, this would be 25% (same as other columns)
    // For fewer time slots, it will be proportionally smaller
    return props.$timeSlots > 0 ? `${props.$timeSlots * 6.25}%` : '0';
  }};
`;
...
const WeatherDataCell = styled(TableCell)`
  ...
  .weather-card {
    ...
    width: 25%; /* Each card takes 25% of the cell width for 4 time slots */
    box-sizing: border-box;
    overflow: hidden; /* Hide overflow */
    padding: 0 1px; /* Reduce padding to fit more content */
    height: 100%; /* Ensure full height */
    display: flex;
...
const TodayDataCell = styled(WeatherDataCell)`
  ...
  .weather-card {
    flex: 1;
    min-width: 0; /* Allow cards to shrink if needed */
    max-width: calc(100% / ${props => props.$timeSlots || 1}); /* Ensure each card takes equal width */
    width: calc(100% / ${props => props.$timeSlots || 1}); /* Ensure each card takes equal width */
    box-sizing: border-box;
    overflow: hidden; /* Hide overflow */
...
// Media query breakpoint for mobile view
const MOBILE_BREAKPOINT = '768px';

// Wrapper to conditionally show desktop or mobile view
const ResponsiveWrapper = styled.div`
  /* Desktop view is shown by default */
  .desktop-view {
    display: block;
  }

  /* Mobile view is hidden by default */
  .mobile-view {
    display: none;
  }

  /* On smaller screens, switch views */
...
const TimeseriesDisplay = ({
  ...
  if (activeTab === '3-days') {
    ...
    // Create a new array for the 7-days tab
    displayForecast = [];
    ...
    // Start from day 4 (after the 3-days tab which shows today + 3 days)
    for (let i = 4; i < 11; i++) {
      ...
      // Try to find this day in the forecast data
      const forecastDay = forecast.find(day => day.dayIndex === i);

      if (forecastDay) {
        // If we found this day in the forecast data, use it but update the day name and date
        const updatedDay = {
          ...forecastDay,
          day: dayName,
          date: dateStr,
        ...
        displayForecast.push(updatedDay);
      } else {
        // If this day is not in the forecast data, create a placeholder

        // Create a new day with placeholder data
        const newDay = {
          day: dayName,
          date: dateStr,
          hourlyForecasts: [],
        ...
        // Add standard time slots with placeholder data
        const standardTimeSlots = [3, 9, 15, 21];
        newDay.hourlyForecasts = standardTimeSlots.map(hour => ({
          hour: `${hour.toString().padStart(2, '0')}`,
          temperature: '--',
          minTemperature: '--',
          feelsLike: '--',
        ...
        displayForecast.push(newDay);
      }
    }


    // Always use 7 columns for the 7-days tab
    columnsToShow = 7;

    firstDayTimeSlots = 4; // All days in 7-days tab have 4 time slots

    // Ensure we have hourly forecasts for each day
    displayForecast.forEach((day, index) => {
      ...
      // If we have some hourly forecasts but not all 4 standard ones, add placeholders for missing ones
        ...
        if (missingHours.length > 0) {
          // Add placeholders for missing time slots with '--' for all missing data
          const placeholders = missingHours.map(hour => {
            // Log that we're adding a placeholder for missing data
